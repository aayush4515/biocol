(function () {
  ...
  (function initRunControls() {
    ...
    function fillPlateauWindow() {
      ...
      plateauEl.innerHTML = "";
      // Backend requires plateau_window >= 2; options are 2..max-1 (or just 2 if max <= 3)
      var minPlateau = 2;
      var maxPlateau = Math.max(minPlateau, max - 1);
      for (var j = minPlateau; j <= maxPlateau; j++) {
      ...
      if (plateauEl.selectedIndex < 0 && plateauEl.options.length) {
        var defaultVal = Math.min(10, maxPlateau);
