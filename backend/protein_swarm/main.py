...
def design(
    ...
    output_dir: str = typer.Option("backend/outputs", "--output-dir", help="Directory for output artefacts"),
    ...
    # ── Prompt debug flags ───────────────────────────────────────────────
    dump_prompts: bool = typer.Option(False, "--dump-prompts", help="Dump LLM prompts to backend/outputs/debug/prompts/ (requires --debug)"),
) -> None:
    ...
    if dump_prompts:
        console.print(f"  Prompt dump     : ON (backend/outputs/debug/prompts/)")
...
def dashboard(
    ...
    import uvicorn
    # Dashboard lives under frontend/; add it so uvicorn can load dashboard.server
